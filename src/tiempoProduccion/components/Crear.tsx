...
export const Crear = () => {
  ...
  const { register, handleSubmit , formState:{errors}, setValue} = useForm<RegistarTiempoProduccionI>()
  useEffect(() => {
    (async () => {
      try {
...
<div className="flex justify-center items-center flex-col mt-10 px-4">
  ...
  >
    ...
    <div>
      ...
      <input {...register("recepcion", { valueAsNumber: true, min: 0, required: true })} type="number" className="input" />
      {errors.recepcion && <Alert severity="error">Ingresa el valor</Alert>}
    </div>

    <div>
      <label className="label">Almacén</label>
      <input {...register("almacen", { valueAsNumber: true, min: 0, required: true })} type="number" className="input" />
      {errors.almacen && <Alert severity="error">Ingresa el valor</Alert>}
    </div>

    <div>
      <label className="label">Digital</label>
      <input {...register("digital", { valueAsNumber: true, min: 0, required: true })} type="number" className="input" />
      {errors.digital && <Alert severity="error">Ingresa el valor</Alert>}
    </div>

    <div>
      <label className="label">Bisel</label>
      <input {...register("bisel", { valueAsNumber: true, min: 0, required: true })} type="number" className="input" />
      {errors.bisel && <Alert severity="error">Ingresa el valor</Alert>}
    </div>

    <div>
      <label className="label">Tinte</label>
      <input {...register("tinte", { valueAsNumber: true, min: 0, required: true })} type="number" className="input" />
      {errors.tinte && <Alert severity="error">Ingresa el valor</Alert>}
    </div>

    <div>
      <label className="label">Cálculo</label>
      <input {...register("calculo", { valueAsNumber: true, min: 0, required: true })} type="number" className="input" />
      {errors.calculo && <Alert severity="error">Ingresa el valor</Alert>}
    </div>

    <div>
      <label className="label">Antirreflejo</label>
      <input {...register("antireflejo", { valueAsNumber: true, min: 0, required: true })} type="number" className="input" />
      {errors.antireflejo && <Alert severity="error">Ingresa el valor</Alert>}
    </div>

    <div>
      <label className="label">Despacho</label>
      <input {...register("despacho", { valueAsNumber: true, min: 0, required: true })} type="number" className="input" />
      {errors.despacho && <Alert severity="error">Ingresa el valor</Alert>}
    </div>

    <div>
      <label className="label">Control de Calidad</label>
      <input {...register("controlCalidad", { valueAsNumber: true, min: 0, required: true })} type="number" className="input" />
      {errors.controlCalidad && <Alert severity="error">Ingresa el valor</Alert>}
    </div>

    <div>
      <label className="label">Tiempo Logística Entrega</label>
      <input {...register("tiempoLogisticaEntrega", { valueAsNumber: true, min: 0, required: true })} type="number" className="input" />
    ...
    <div>
      <label className="label">Antirreflejo</label>
      <select {...register("estadoAntireflejo", { required: true })} className="input">
        <option value="">Selecciona</option>
    ...
    <div>
      ...
      <select {...register("estadoLente", { required: true })} className="input">
        <option value="">Selecciona</option>
    ...
    <div>
      ...
      <select {...register("estadoProeceso", { required: true })} className="input">
        <option value="">Selecciona</option>
    ...
    <div>
      ...
      <select {...register("combinacionTiempo", { required: true })} className="input">
    ...
    <div>
      ...
      <select {...register("sucursal", { required: true })} className="input">
  ...
  {isOpen && <TiempoCombinacionModal closeModal={closeModal} isOpen={isOpen} openModal={openModal} />}
</div>
