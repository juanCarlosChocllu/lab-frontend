...
export function RegistrarTiempoProduccionPage() {
  const [selectedCombinacion, setSelectedCombinacion] = useState("");
  ...
  console.log(selectedCombinacion);
  ...
  const onSubmit = async (data: RegistarTiempoProduccionI) => {
    try {
      ...
      if (selectedCombinacion) {
        data.combinacionTiempo = selectedCombinacion;
      } else {
        toast.error("Seleccione un combinacion");
      }
  ...
  const filteredCombinaciones = Combiancion.filter((combo) =>
    `${combo.tipoLente} ${combo.tipoColor} ${combo.tratamiento}`
      .toLowerCase()
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-96">
            <div className="bg-white p-6 rounded-2xl shadow-2xl border border-gray-200 sticky top-4">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3 max-h-[600px] overflow-y-auto">
                  {filteredCombinaciones.length > 0 ? (
                    filteredCombinaciones.map((combo) => (
                      <div
                        key={combo._id}
                        onClick={() => setSelectedCombinacion(combo._id)}
                        className={`p-4 rounded-lg  transition-all cursor-pointer ${
                          selectedCombinacion === combo._id
                            ? "bg-blue-100 border-blue-500 shadow-lg"
                            : "bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200 hover:shadow-md hover:border-blue-300"
                        }`}
                      >
                        ...
                        <div className="text-gray-600 text-xs mt-1">
                          {combo.tipoColor} • {combo.tratamiento}
                        </div>
                        {selectedCombinacion === combo._id && (
